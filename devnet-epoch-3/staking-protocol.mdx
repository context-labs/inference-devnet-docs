...
- Manages USDC revenue sharing with delegators
...
The epoch reward emissions scheduled is encoded and enforced in our program when epochs are finalized. You can view the [reward emissions schedule here](https://github.com/context-labs/inference-staking/blob/main/programs/inference-staking/src/emissions.rs).
...
- Operators can set a USDC commission rate (0-100%) separate from token reward commission
- USDC revenue is shared proportionally among all pool participants based on their stake
- Shared USDC accrues on-chain when rewards are distributed to the pool
- Delegators earn their proportional share of USDC revenue after operator commission
- All USDC earnings (operator commission and staked share) can be withdrawn at any time without affecting stake

<Note>
  USDC revenue sharing allows delegators to participate in the network's revenue
  stream, not just token rewards. This creates stronger economic alignment
  between operators and delegators.
</Note>
...
- **Registration Fee**: A one-time fee paid in $INT-DEV tokens to prevent spam and bad actor activity
- **Minimum Stake**: Operators must stake a minimum amount before delegations are accepted
- **Unique Wallet**: Each wallet can create at most one operator pool
...
For more details, see the [Network Security](/devnet-epoch-3/network-security) page.
...
**Q: How do USDC withdrawals work?**<br/>
...
**Q: Can I create multiple operator pools?**<br/>
A: No. Each wallet can create at most one operator pool to ensure fair participation and prevent gaming of the system.

**Q: What happens to the registration fee?**<br/>
A: The registration fee is paid to the protocol and helps prevent spam or malicious pool creation.

**Q: What is the base unit of the $INT token called?**<br/>
A: The Inference $INT network token has 9 decimal places of precision and a single unit is referred to as a Bozeman.
