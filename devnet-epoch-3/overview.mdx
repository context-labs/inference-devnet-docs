...
New workers are placed in the evaluation lane and receive test traffic to build trust. This traffic does not earn rewards/points. Once a worker has passed a specific number of verifications, they are promoted to the trusted lane and begin receiving verified inference traffic.

If a trusted worker begins failing too many verifications, they can be moved back to the evaluation lane until performance improves.

**Verification Thresholds**

Note that these thresholds may be adjusted over time. The following are an example:
...
<Warning>
  ...
  guidelines](/devnet-epoch-3/network-security).
</Warning>
...
- The network periodically checks whether instances are by sending a health check inference request to the instance.
...
<Tip>
  ...
  allow us to incentive specific types of hardware to join the network.
</Tip>
